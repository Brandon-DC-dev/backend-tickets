...
//            del path (el cliente sólo envía { observacion, gestor_id }).
//
...
// --- POST /ticket-observaciones/por-ticket/:ticketId ----------------------
// Body: { observacion: string, gestor_id?: string }
//
...
router.post(
  ...
  async (req, res, next) => {
    try {
      const { observacion, gestor_id } = req.body ?? {};
...
router.use(
  ...
  createCrudRouter({
    ...
    `,
    allowedFilters: ['ticket_id', 'gestor_id'],
